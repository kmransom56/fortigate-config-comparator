...
import sqlite3
import json
from config import FMGR_URL, DB_NAME, LOG_LEVEL, SSL_VERIFY, LOG_FILE, FMGR_USERNAME, FMGR_PASSWORD
import traceback
import logging
...
LOG_FILE = os.getenv('LOG_FILE', './app.log')  # Get from environment
LOG_LEVEL = os.getenv('LOG_LEVEL', 'INFO')  # Get from environment
...
file_handler.setLevel(getattr(logging, LOG_LEVEL))  # Set level from environment

# Console handler
console_handler = logging.StreamHandler()
console_handler.setFormatter(log_formatter)
console_handler.setLevel(getattr(logging, LOG_LEVEL))  # Set level from environment

# Get the root logger
logger = logging.getLogger()
logger.setLevel(getattr(logging, LOG_LEVEL))  # Set level from environment
...
def export_ips():
    ...
    if status == 200:
        ...
        """, message=message)
@app.route('/fetch_ips', methods=['GET'])
def fetch_ips():
    """Endpoint to fetch IPs for VLAN 10."""
    ...
    try:
        ...
        response = requests_retry_session().post(FMGR_URL, json=query_payload, verify=SSL_VERIFY)
        ...
        # Check if VLAN 10 IPs are found
        if not json_response.get('result'):
            logger.warning("No VLAN 10 IPs found for any device")
            return render_template_string("""
            <h1>No VLAN 10 IPs Found</h1>
            <p>No IPs were found for VLAN 10 on any device.</p>
        ...
        logger.error(f"Error in fetch_vlan_10_ips: {str(e)}")
        return render_template_string("""
        <h1>Error</h1>
        <p>{{ error }}</p>
        <a href="{{ url_for('index') }}">Back to Home</a>
